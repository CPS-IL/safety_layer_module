...
DecisionComponent::DecisionComponent()
    : reader_chassis_(nullptr),
      reader_control_command_(nullptr),
      reader_detections_mission_(nullptr),
      reader_detections_safety_(nullptr),
      reader_mission_layer_trajectory_(nullptr),
      writer_control_command_(nullptr),
      channel_name_reader_chassis_("/apollo/canbus/chassis"),
      channel_name_reader_control_command_("/apollo/control"),
      channel_name_reader_detections_mission_("/apollo/perception/obstacles"),
      channel_name_reader_detections_safety_(
          "/apollo/safety_layer/lidar/depth_clustering/detections"),
      ...
      localization_position_(0, 0, 0),
      velocity_(0, 0, 0),
      accel_(0, 0, 0),
      override_(false),
      braking_acceleration_(7.0),
      chassis_speed_mps_(0),
      control_command_brake_(100),
...
{
	return (distance * 0.95) - 0.1; // safety margin, as defined in paper
}
	...
	const std::vector<double> overlaps, const std::shared_ptr<planning::ADCTrajectory> mission_layer_trajectory_message)
{
	...
	// Though we use only one vehicle so values remain constant.
	Eigen::Vector3d ego_extent;
	ego_extent.x() = 5.02;
	ego_extent.y() = 2.13;
	ego_extent.z() = 1.50;	
	double braking_distance;
	braking_distance = (chassis_speed_mps_ * control_latency_); // override decision to braking start distance
    ...
    if (FN_INJECT && safety_closest_points.size() > 0)
	{
        return true;
    }
	...
	// TODO: This assumes scenario information, i.e. stationary obstacle.
	// Make it generic for future work.
	for (unsigned int i = 0; i < safety_closest_points.size(); i++)
	{
		...
		// Assumption: Ego vehicle planned trajectory upto time to stop is available.		
		for (const auto& trajectory_point : mission_layer_trajectory_message->trajectory_point())
		{
			...
			// Here we are comparing the distance to 10 cm, but that's valid only because obstacle is stationary.
...
void
DecisionComponent::createLogFileSpeed()
{
	if (!log_speed_)
    {
        return;
    ...
    log_file_speed_.open(log_file_name_speed_, std::fstream::out | std::fstream::trunc);

    if (!log_file_speed_.is_open() || !log_file_speed_.good())
    {
        AERROR << "Failed to open speed log file.";
    }
}

void
DecisionComponent::logSpeed()
{
	if (!log_speed_)
    {
        return;
    ...
    if (log_file_speed_.is_open() && log_file_speed_.good())
    {
        log_file_speed_ << chassis_speed_mps_ << std::endl;
    }
    else
    {
        AERROR << "Invalid speed log file stream.";
    }
}
